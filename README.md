...
[![CI](https://github.com/neonpilot/neonpilot/actions/workflows/ci.yml/badge.svg)](https://github.com/neonpilot/neonpilot/actions/workflows/ci.yml)
[![License: Apache-2.0](https://img.shields.io/badge/License-Apache--2.0-blue.svg)](./LICENSE)
[![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-blue)](./pyproject.toml)

Submission for the **Arm AI Optimization Challenge 2026** (Devpost), **Mobile AI** track.

> Status: milestones M0–M4 (scaffold, probe, bench harness, staged search, report + preset)
> are implemented and tested. M5 (real M1 Max + M5 measured presets) is the remaining
> milestone — see the placeholders in [Results](#results) below.
...
<!-- RESULTS:M1MAX -->
> **Apple M1 Max — to be filled from the real M5-milestone benchmark run.** The table skeleton
> below matches the fields `neonpilot report`/`presets/*.json` actually emit
> (`baseline_gen_ts`, `tuned_gen_ts`, `speedup_gen_pct`, etc. — see [`ARCHITECTURE.md`](./ARCHITECTURE.md)
> for the full `Preset`/`SweepResult` schema). Do not fill in numbers here by hand — regenerate
> from a real `neonpilot optimize` → `report` → `apply` run and paste the resulting figures.

| Metric | Baseline (llama.cpp defaults) | Tuned | Speedup |
|---|---|---|---|
| Generation t/s (median ± stddev) | _TBD_ | _TBD_ | _TBD_ |
| Prefill t/s (median ± stddev) | _TBD_ | _TBD_ | _TBD_ |
| Winning config | — | _threads=?, kv=?, flash_attn=?, batch/ubatch=?_ | — |

Reference model: Qwen2.5-3B-Instruct Q4_K_M (~2.1 GB). Reps: ≥3. Full report:
`presets/apple-m1-max/qwen2.5-3b-instruct-q4_k_m.json` + the committed demo `report.html`
(once generated).
<!-- /RESULTS:M1MAX -->
...
Or, once `llama-bench` and a model are in place, run the whole pipeline in one shot:

```bash
make benchmark
```

(`make benchmark` currently runs `fetch-llama` + `probe`; `optimize`/`report`/`apply` wiring in
the Makefile target lands alongside the M5 real-hardware run — until then, invoke those three
commands directly as shown above.)
